...
class ExpMultiGpuTrainer_aux(AbstractTrainer):
    def __init__(self, config, stage="Train"):
        super(ExpMultiGpuTrainer_aux, self).__init__(config, stage)
        np.random.seed(2021)
    ...
    def _train_settings(self, model_cfg, data_cfg, config_cfg):
        ...
        if self.local_rank == 0:
            ...
            self.val_set = load_dataset("Mydata_Val_aux")(val_options)
            # wrapped with data loader
    ...
    def _load_ckpt(self, best=False, train=False):
        ...
        self.model.module.load_state_dict(load_dict["model"])
        ...
        try:
            ...
            for epoch_idx in range(start_epoch, self.num_epoch + 1):
                ...
                for batch_idx, train_data in train_generator:
                    ...
                    self.model.train()
                    I, A, Y = train_data
                    I, A = self.train_loader.dataset.load_item(I, A)
                    in_I = I.to('cuda:0')
                    in_A = A.to('cuda:0')
                    Y = torch.tensor(tuple(list(map(int, list(Y))))).to('cuda:0')
                    ...
                    with autocast():
                        Y_pre = self.model(in_I, in_A)

                        # for BCE Setting:
                        if self.num_classes == 1:
                            Y_pre = Y_pre.squeeze()
                            loss = self.loss_criterion(Y_pre+1e-6, Y.float())
                        ...
                        recons_loss  = exp_recons_loss(self.model.module.loss_inputs['recons1'], (in_I, Y))
                        contra_loss = self.contra_loss(self.model.module.loss_inputs['contra1'], Y)
                    ...
                    self.recons_loss_meter1.update(reduce_tensor(recons_loss).item())
                    self.contra_loss_meter1.update(reduce_tensor(contra_loss).item())
                    ...
                    if self.local_rank == 0:
                        ...
                        print( "Train Epoch %d (%d/%d), Global Step %d, Loss %.4f, Recons %.4f, Contra %.4f, "
                            "ACC %.4f, LR %.6f" % (
                                epoch_idx, batch_idx, len(self.train_loader), global_step,
                                self.loss_meter.avg, self.recons_loss_meter.avg, 
                                self.contra_loss_meter.avg, 
    ...
    def validate(self, epoch, step, timer, writer):
        ...
        with torch.no_grad():
            ...
            for val_idx, val_data in val_generator:
                I, A, Y = val_data
                I, A = self.val_loader.dataset.load_item(I, A)
                # in_I, Y = self.to_device((I, Y))
                in_I = I.to('cuda:0')
                in_A = A.to('cuda:0')
                Y = torch.tensor(tuple(list(map(int, list(Y))))).to('cuda:0')
                ...
                if val_idx == v_idx or val_idx == 1:
                    sample_recons = list()
                    for _ in self.model.module.loss_inputs['recons1']:
